...
namespace mc {
	...
	public:
        ...
        void setBlock(BlockPos* pos, uint32_t state, int id, bool unk) {
			if(state == 0)
			{
				state = mc::Block::byId(id)->defBlockState;
			}
            code::Func<void, 0x02547678, Level*, BlockPos*, uint32_t, int, bool>()(this, pos, state, id, unk);
        }
        ...
        mstd::vector<mc_boost::shared_ptr<mc::BlockEntity>> blockEntities;
